...
async def upload_img(ctx, img: Image, fname: str):
   fp = io.BytesIO()
   img.save(fp, format='png')

   # seek to beginning of saved image data
   fp.seek(0)
   await ctx.followup.send("Your generated image:", file=discord.File(fp, filename=fname))
   fp.close()
...
@bot.command()
async def gen_time_stats(ctx, system: rt.system_t, compass_history_csv: discord.Attachment, img_width: discord.Option(input_type=int, descrption="width of the generated image, in pixels", name="img_width") = 800):
   await ctx.response.defer()
   contents = await compass_history_csv.read()
   contents = contents.decode('utf-8')
...
@bot.command()
async def gen_month_stats(ctx, system: rt.system_t, compass_history_csv: discord.Attachment, img_width: discord.Option(input_type=int, descrption="width of the generated image, in pixels", name="img_width") = 800):
   await ctx.response.defer()
   contents = await compass_history_csv.read()
   contents = contents.decode('utf-8')
